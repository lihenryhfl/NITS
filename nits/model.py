...
#     def pdf(self, x, params, return_intermediaries=False):
...
#     def __init__(self, d, arch, start=-2., end=2., A_constraint='neg_exp',
#                  monotonic_const=1e-2, final_layer_constraint='softmax',
#                  autoregressive=True, pixelrnn=False):
#         super(ConditionalNITS, self).__init__()
...
#             pre_activations, As, bs, nonlinearities, residuals = [], [], [], [], []
#         for i in range(self.d):
#             params_ = self.get_params(i, params)
#             func = getattr(self.nits_list[i], func_name)
#             if func_name == 'icdf':
#                 if self.autoregressive:
#                     given_x = torch.cat(results + [torch.zeros(len(x), self.d - len(results), device=params.device)], axis=1)
...
#             else:
#                 x_masked = self.causal_mask(x, i)
#                 if func_name == 'cdf' or func_name == 'forward_':
#                     result = func(x_masked, params_, x_unrounded=x_unrounded, return_intermediaries=return_intermediaries)
#                 else:
#                     result = func(x_masked, params_, return_intermediaries=return_intermediaries)
...
#     def backward_(self, x, params):
#         return self.apply_conditional_func('backward_', x, params)

#     def cdf(self, x, params, x_unrounded=None, return_intermediaries=False):
#         return self.apply_conditional_func('cdf', x, params, x_unrounded=x_unrounded, return_intermediaries=return_intermediaries)

#     def pdf(self, x, params):
#         return self.apply_conditional_func('pdf', x, params)
...
# #     def backward_(self, x, params, return_intermediaries=False):
# #         y, pre_activations, As, bs, nonlinearities, residuals = self.forward_(x, params, return_intermediaries=True)
...
# #             start_idx, end_idx = 10 + i * self.n_params, 10 + (i + 1) * self.n_params
# # #             return torch.cat([params[:,start_idx:end_idx], params[:,:10]], axis=1)
# #             tmp = torch.cat([params[:,start_idx:end_idx], params[:,:10]], axis=1)
# #             return tmp
# #         else:
# #             start_idx, end_idx = i * self.n_params, (i + 1) * self.n_params
# #             return params[:,start_idx:end_idx]

# #     def apply_conditional_func(self, func_name, x, params, given_x=None, return_intermediaries=False):
# #         n = max(len(x), len(params))
# #         results = []
# #         if return_intermediaries:
# #             pre_activations, As, bs, nonlinearities, residuals = [], [], [], [], []
...
# #     def forward_(self, x, params, return_intermediaries=False):
# #         return self.apply_conditional_func('forward_', x, params, return_intermediaries=return_intermediaries)

# #     def backward_(self, x, params, return_intermediaries=False):
# #         return self.apply_conditional_func('backward_', x, params, return_intermediaries=return_intermediaries)

# #     def cdf(self, x, params, return_intermediaries=False):
# #         return self.apply_conditional_func('cdf', x, params, return_intermediaries=return_intermediaries)
